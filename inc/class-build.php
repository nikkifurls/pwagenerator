...
class Build {
	...
	private function set_build_version(bool $increment = false): void {
		...
		if (empty($file_contents)) {
			CLI::display_error("Failed to set build version: contents could not be extracted from {$service_worker_file}");
			exit;
		}
		...
		if (!empty($increment)) {
			...
			if (empty($position)) {
				CLI::display_error("Failed to set build version: project version could not be found in {$service_worker_file}");
				exit;
			}
	...
	private function compile_files(): void {
		...
		$project_reflection_class = new ReflectionClass('Project');
		...
		if (!empty(Project::$fontawesome)) {
			...
			foreach ($directories as $directory_path) {
				if (!is_dir($directory_path)) {
					...
					if (!is_dir($directory_path)) {
						CLI::display_error("Failed to process font files: cound not create {$directory_path} directory");
						exit;
					}
	...
	private function generate_manifest(): void {
		...
		if (!empty(Project::$android_app_id) || !empty(Project::$apple_app_id)) {
			...
			} else if (!empty(Project::$android_app_id['free'])) {
			...
			} else if (!empty(Project::$apple_app_id['free'])) {
	...
	private function generate_favicons(): void {
		...
		file_put_contents(Project::$directory_path . '/favicon_config.json', json_encode($favicon_data_config, JSON_PRETTY_PRINT));
